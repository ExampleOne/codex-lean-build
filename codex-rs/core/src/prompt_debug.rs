...
/// Debug helper: return the EXACT `(instructions, tools_json)` that would be sent
/// to the Responses API for one turn, so the wire-format token cost can be measured
/// without estimating. `tools_json` is the serialized tools array (pretty-printed).
#[doc(hidden)]
pub async fn build_request_debug(
    mut config: Config,
    input: Vec<UserInput>,
    state_db: Option<StateDbHandle>,
    ...
    let output =
        build_request_debug_from_session(thread.thread.codex.session.as_ref(), input).await;
    let shutdown = thread.thread.shutdown_and_wait().await;
    let _removed = thread_manager.remove_thread(&thread.thread_id).await;
...
pub(crate) async fn build_request_debug_from_session(
    sess: &Session,
    input: Vec<UserInput>,
) -> CodexResult<(String, String)> {
    let turn_context = sess.new_default_turn().await;
