...
2. Build with the `shell` tool: `lake build` (or `lake env lean <file>` for a single file).
3. Read the compiler output, fix the first real error, repeat.
4. You are done when the build succeeds and `grep -rn "sorry\|admit" <targets>` is empty.
